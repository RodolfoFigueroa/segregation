...
def res2pd(r):
    return pd.DataFrame.from_dict({k: [v] for k, v in flatten_res(r).items()})
...
def get_seg_full(
    ...
    ds["seed"] = seed_xr
    if write_to_disk:
        ds.to_netcdf(path=out_path / "contingency_tables.nc", engine="netcdf4")

    # Create a df of individuals from survey dataframe
    # with multi index on linking categories
    df_ind = df_survey.set_index(list(df_survey.columns.drop("Ingreso_orig")))
    ...
    median_MZ = ipf.weighted_mean(df_ind.Ingreso_orig.values, df_ind.w_MZ.values)
